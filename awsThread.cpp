...
void awsThread(void)
{
    ...
    AWSIoTClient AWSClient(wifi, AWSIOT_THING_NAME , SSL_CLIENTKEY_PEM, strlen(SSL_CLIENTKEY_PEM), SSL_CLIENTCERT_PEM, strlen(SSL_CLIENTCERT_PEM));
    ...
    AWSClient.subscribe(ep, AWSIOT_TOPIC, AWS_QOS_ATMOST_ONCE, messageArrived);
    ...
    while(1)
    {
        ...
        if(doPublish)
        {
            ...
            AWSClient.publish(ep,"updateTemp", buffer, strlen(buffer),  publish_params);
        }
        doPublish = false;
    }
}
